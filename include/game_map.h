...
public:
    ...
    int width();
    int height();
    std::vector<std::unique_ptr<Room>> generate_rooms();
    void dfs(int i, int j, std::vector<std::vector<int> >& visited,
                   std::vector<Node *>& connected_component);
    void dfs_room(std::unique_ptr<Room> room, std::vector<std::unique_ptr<Room>>& rooms, 
        std::vector<std::unique_ptr<Room>>& visited, std::vector<std::unique_ptr<Room>>& connected_component);
    std::vector<std::vector<std::unique_ptr<Room>> > group_rooms_into_connected_components(
                                std::vector<std::unique_ptr<Room>>& rooms);
    void connect_main_room_to_other_rooms(std::unique_ptr<Room> main_room, std::vector<std::unique_ptr<Room>>& rooms,
    ...
    void connect_rooms(std::vector<std::unique_ptr<Room>>& rooms);
    void dfs_simple(int i, int j, std::vector<std::vector<int> >& visited,
                    std::vector<std::pair<int, int> >& connected_component);
    int count_connected_componenets(std::vector<std::unique_ptr<Room>>& rooms);
    void remove_connected_components(std::vector<std::unique_ptr<Room>>& rooms);
